...
### 3. Внесения изменений с GitHub на локальный репозиторий
Внесение изменений в локальный репозиторий, залитый до изменений на GitHub вносятся с помощью команды 
```
git pull
```

### 4. Участие в проекте на GitHub (pull request)
Что бы поучавствовать в каком-нибудь проекте на GitHub, необходимо зайти в репозиторий проекта, в котором хотим поучавствовать и скопировать его на свой аккаунт. Для этго нужно нажать Fork (это скопирует репозиторий к себе)
Далее форкнутый репозиторий клонируем на локальный компьютер.

В каждом проекте должен быть файл README.md 

Работать с чужим проектом нужно в своей новой ветке.

После выполения изменений и окончания работы с проектом отправляем изменения через "push". В GitHub появляется кнопка  "Compare & pull request" - нажимаем, делаем описание и отправляем владельцу.
