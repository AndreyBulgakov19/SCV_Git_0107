...
Для последующего сохранения изменений в удаленный репозиторий необходимо будет использовать команду:
```
git push
```
Эту команду использовать только после фиксации изменений
...
Мы так же можем можем изменять и фиксировать работу на самом удаленном репозитории.

1. Открыть нужный нам репозиторий на сайте Github
2. Открыть файл который мы хотим изменить
3. Правее нажать на картинку карандаша и ввести изменения
4. Прокрутить вниз, дать название коммита и нажать на кнопку "`Commit changes`"

## 22. Работа с PULL REQUEST

Для того чтобы работать с чужим удаленным репозиторием и предлагать свои изменения, нам необхходимо:

1. Открыть нужный чужой репозиторий
2. Справа  сверху нажать на кнопку "`Fork`" (это позволит нам скопировать репозиторий в наш профиль со ссылкой на источник)
3. Копируем ссылку скопированного репозитория с нашего профиля и создаем его на компьютере (смотри раздел 19)
4. Создаем и переходим новую ветку в созданном репозитории
5. Вносим изменения какие хотели и фиксируем их(все делаем в новой ветке)
6. Вводим команду
```
git push --set-upstream origin ИмяНовойВетки
```
так же можно ввести просто:
```
git push
```
И если команда не сработает то git скажет нам что надо ввести чтобы все получилось

7. Возвращаемся в наш скопированный реепозиторий на сайте Github
8. Нажимаем на зеленую кнопку "`Compare & pull request`"
9. Дальше Githhub проверит можно ли слить наш репозиторий с источником
10. И после этого предложит добавить немного описания
11. Нажимаем на зеленую кнопку "`Create pull request`"

После этого в аккаунте у которого мы делали копию получит эти изменения и решит, добавлять ли их.